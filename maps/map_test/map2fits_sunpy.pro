; 09-Aug-2017 IGH
; Tweak to original map2fits.pro as the fits files it produces don't work nicely with sunpy
; 
; Specifically:
; Makes units 'arcsec' now as 'arcsecs' breaks sunpy
; Force time to standard format, as some inputs fits (i.e. SXI) can use formats sunpy won't understand
; Added xcen and ycen as sunpy likes having those (though crval1,crval2 might be enough)
; Renamed 'rsun' to 'rsun_obs as otherwise get sunpy warning
; Added in 'dsun_obs' as otherwise get sunpy warning
; Added in observer lat and long, as otherwiser get sunpy warning
;
;~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
;+
; Project     : SOHO-CDS
...
pro map2fits_sunpy,map,file,err=err,byte_scale=byte_scale,verbose=verbose
  ...
  for i=0,np-1 do begin
    ...
    ; IGH Changed from arcsecs
    fxaddpar, header, 'cunit1', 'arcsec','Arcseconds from center of Sun'
    fxaddpar, header, 'cunit2', 'arcsec','Arcseconds from center of Sun'
    ;IGH added in xcen,ycen
    fxaddpar, header, 'xcen', xcen, 'Centre X'
    fxaddpar, header, 'ycen', ycen, 'Centre Y'
    ...
    ; IGH added in the oberver location
    fxaddpar, header, 'HGLT_OBS',0,'Observer latitude'
    fxaddpar, header, 'HGLN_OBS',0,'Observer longitude'


    if use_rtime then obs_time=map[i].rtime else obs_time=map[i].time

    ; IGH make sure the time is in a standard format
    obs_time=anytim(obs_time,/ccsds)
    ...
    ; IGH renamed rsun to rsun_obs
    if tag_exist(map[i],'RSUN') then fxaddpar,header,'RSUN_OBS',map[i].rsun,'Solar radius (arcsecs)'

    ; IGH added in Sun Earth distance as well
    tempaa=get_sun(obs_time,dist=dist)
    fxaddpar,header,'DSUN_OBS',dist*149597870700.,'Solar distance (meter)'
